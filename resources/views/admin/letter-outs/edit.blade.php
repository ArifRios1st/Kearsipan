...
@section('content')
    <div class="card">
        <div class="card-header">
            {{ trans('global.edit') }} {{ trans('cruds.letter-out.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route('admin.letter-outs.update', $letterOut->id) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('code') ? 'is-invalid' : '' }}" type="text" name="code"
                        id="code" value="{{ old('code', $letterOut->code) }}" required>
                    @if ($errors->has('code'))
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('receiver') ? 'is-invalid' : '' }}" type="text" name="receiver"
                        id="receiver" value="{{ old('receiver', $letterOut->receiver) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('regarding') ? 'is-invalid' : '' }}" type="text" name="regarding"
                        id="regarding" value="{{ old('regarding', $letterOut->regarding) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control date {{ $errors->has('sended_at') ? 'is-invalid' : '' }}" type="text"
                        name="sended_at" id="sended_at" value="{{ old('sended_at', $letterOut->sended_at) }}" required>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('servicer') ? 'is-invalid' : '' }}"
                        ...
                        @foreach ($servicers as $id => $entry)
                            <option value="{{ $id }}" {{ old('servicer_id', $letterOut->servicer->id) == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
